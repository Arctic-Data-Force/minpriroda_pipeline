...
class VitInference:
    """
    Class for performing inference using ViTPose models with YOLOv8 or RT-DETR object detection and SORT tracking.

    Args:
        model (str): Path to the ViT model file (.pth, .onnx, .engine).
        yolo (str): Path of the YOLOv8 or RT-DETR model to load.
        detection_model_type (str, optional): Type of the detection model.
            Valid values are 'yolo' for YOLOv8 and 'rtdetr' for RT-DETR. Defaults to 'yolo'.
        ...
        dataset (str, optional): Name of the dataset. If None, it's extracted from the file name.
                                 Valid values are 'coco', 'coco_25', 'wholebody', 'mpii',
                                              'ap10k', 'apt36k', 'aic'.
        yolo_size (int, optional): Size of the input image for the detection model. Defaults to 320.
        ...
        yolo_step (int, optional): The tracker can be used to predict the bboxes instead of yolo for performance,
                                   this flag specifies how often the detection model is applied (e.g., 1 applies detection every frame).
                                   This does not have any effect when is_video is False.
    """

    def __init__(self, model: str,
                 yolo: str,
                 detection_model_type: str = 'yolo',
        ...
        assert os.path.isfile(yolo), f'The detection model {yolo} does not exist'
        assert detection_model_type in ['yolo', 'rtdetr'], \
            f"Invalid detection_model_type '{detection_model_type}'. Supported types: 'yolo', 'rtdetr'."
        ...
        # Initialize the detection model based on the specified type
        if self.detection_model_type == 'yolo':
            self.detector = YOLO(yolo, task='detect')
        elif self.detection_model_type == 'rtdetr':
            # Assuming RT-DETR can be loaded similarly to YOLO models in Ultralytics
            # Replace 'RT-DETR' with the actual model class if different
            self.detector = RTDETR(yolo)  # Modify if RT-DETR requires a different class
        ...
        # if we picked the dataset switch to correct detection classes if not set
        if det_class is None:
            det_class = 'animals' if dataset in ['ap10k', 'apt36k'] else 'human'
        self.detection_classes = list(range(len(self.detector.names)))
    ...
    def inference(self, img: np.ndarray) -> dict[typing.Any, typing.Any]:
        ...
        # First use the detection model (YOLOv8 or RT-DETR) for detection
        res_pd = np.empty((0, 5))
        results = None
        if (self.tracker is None or
           (self.frame_counter % self.yolo_step == 0 or self.frame_counter < 3)):
            results = self.detector(img[..., ::-1], verbose=False, imgsz=self.yolo_size,
                                   device=self.device if self.device != 'cuda' else 0,
                                   classes=self.detection_classes)[0]
        ...
        for bbox, id in zip(bboxes, ids):
            ...
            scores_bbox[id] = scores[id] # Replace this with avg_keypoint_conf*person_obj_conf. For now, only person_obj_conf from detection is being used.

        if self.save_state:
            self._img = img
            self._detector_res = results
    ...
    def draw(self, show_detection=True, show_raw_detection=False, confidence_threshold=0.5):
        """
        Draw keypoints and bounding boxes on the image.

        Args:
            show_detection (bool, optional): Whether to show detection bounding boxes. Default is True.
            show_raw_detection (bool, optional): Whether to show raw detection bounding boxes. Default is False.
            confidence_threshold (float, optional): Threshold for displaying keypoints based on confidence. Default is 0.5.
        ...
        if self._detector_res is not None and (show_raw_detection or (self.tracker is None and show_detection)):
            img = np.array(self._detector_res.plot())[..., ::-1]

        if show_detection and self.tracker is not None:
